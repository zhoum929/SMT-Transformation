	...
	for(int i=0;i<lnth;i++){
		if(num[i]=='.'){
			...
			for(int j=i+1;j<lnth;j++){
				if(num[j]!='0')sign=true;
			}
			if(sign)return num;
			else return num.substr(0,i);
		}
	}
	return num;
}
...
string output_real_constraint(string row, vector<pair<string,string>> contents,string row_print){
	...
	if(contents.size()==1){
		...
		}else {row_print=row_print+contents[0].first+") ";}
		...
		for(pair<string,string>co_and_var:contents){
			...
			}else {row_print=row_print+co_and_var.first+")";}
		}
		row_print=row_print+") "+to_real(constraints_rhs.at(row))+")";
...
void output_fun(){
	...
	for(pair<string,vector<pair<string,string>>>pa:matrix){
		...
		if(constraints.at(row)=="P")continue;
		...
		if(constraints.at(row)=="N"){
			if(objsense=="MIN")row_print+="<= ";
			else row_print+=">= ";
			
			bool isreal=is_real(constraints_rhs.at(row))|is_real(optresult);
			for(int i=0;i<optresult.size();i++)if(optresult[i]=='/'){
				string s="(/ "+optresult.substr(0,i)+" "+optresult.substr(i+1,optresult.length()-1-i)+")";
				optresult=s;
				break;
			}
		
			for(pair<string,string>pa1:contents){
				isreal|=is_real(pa1.second);
				if(variable_type.at(pa1.first)=="Real")isreal=true;
			}
			if(isreal){constraints_rhs.at(row)="(+ "+to_real(optresult)+" "+to_real(constraints_rhs.at(row))+")";}
			else {constraints_rhs.at(row)="(+ "+optresult+" "+constraints_rhs.at(row)+")";}
			if(isreal){row_print=output_real_constraint(row,contents,row_print);}
			else {row_print=output_int_constraint(row,contents,row_print);}
		...
		else if(constraints.at(row)=="L"){row_print=row_print+"<= ";}
		else if(constraints.at(row)=="G"){row_print=row_print+">= ";}
		bool real_constraint=false;
		if(is_real(constraints_rhs.at(row))){
			real_constraint=true;
		}
		for(pair<string,string>co_and_var:contents){
			if(is_real(co_and_var.second))real_constraint=true;
			if(variable_type.at(co_and_var.first)=="Real")real_constraint=true;
		}
		if(real_constraint){row_print=output_real_constraint(row,contents,row_print);}
		else {row_print=output_int_constraint(row,contents,row_print);}
		//write output
...
int main(){
	cout<<"Please type in the name of mps file without suffix"<<endl;
	cin>>filename;
	cout<<"Please type in the optvalue"<<endl;
	cin>>optresult;
	string s1,s2;
	s1=filename+".mps";
	s2=filename+".smt2";
